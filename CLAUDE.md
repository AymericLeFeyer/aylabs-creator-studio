...
- **Dans le Gantt, la couleur dit la chaîne et le texte dit l'état.** Écrire le nom de la chaîne dans la barre serait redondant avec sa couleur, alors que l'avancement ne se lit nulle part ailleurs sur cette vue. La barre entière est un lien vers la fiche : c'est la cible la plus large de la ligne. Le texte prend sa couleur de `readableTextColor(fond)`.
...
- **Dans « Prochains créneaux », le titre est l'étape** (`stepName`), l'intitulé libre venant en dessous : c'est l'étape qui dit ce qu'on va faire, l'intitulé ne fait que préciser.
