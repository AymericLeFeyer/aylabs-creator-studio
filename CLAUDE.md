...
`ManageProductions` possède les écritures : changer `channelId` ou `videoId` **re-synchronise** les revenus de tous les produits et sponsos rattachés (voir contrainte 6). `publish(id, videoId)` rattache la sortie et passe en `done`, **sans cocher aucune étape** — pas même `publication` : on publie souvent avant que tout soit fini, et cocher à la place de l'utilisateur mentirait sur l'avancement. Le reste à faire est rappelé par l'alerte `production_incomplete`.

### `productionNote` — les notes d'une vidéo, en fichiers

`ProductionNote { id, productionId, title, content, createdAt, updatedAt }` — table
`production_notes` (migration 34), `ON DELETE CASCADE` sur la production.

**Plusieurs petites notes par vidéo** (angle, références, retours d'un relecteur), là où
l'ancien champ unique `productions.notes` mêlait tout dans un bloc qu'on ne relisait plus.
Le champ a **quitté le formulaire « Modifier »** (`ProductionDialog`) et le contrat
(`Production.notes` n'existe plus, ni côté API ni côté front). La colonne reste en base,
**morte** : la supprimer imposerait une reconstruction de table. La migration 34 a repris
chaque `notes` non vide dans une note « Notes », convertie en HTML **en SQL** (échappement,
`

`→ paragraphe,`
`→`<br>`) — laissé en texte brut, `toEditorHtml`l'aurait passé
à`marked`, qui fond chaque retour à la ligne dans un seul paragraphe.

- Le contenu est du **HTML**, écrit avec le `ScriptEditor` du script (enregistrement
  automatique, Ctrl+S) en `scriptTools={false}` : ni gabarits, ni angles, ni mode lecture.
  `ScriptEditorView` accepte désormais `scriptTools`, `placeholder` et `minHeight`.
- `title` n'est jamais `null` ; vide, l'écran affiche « Sans titre » (`noteTitle`). Il se
  valide **à la sortie du champ**, comme `StepsPage`.
- Tri par **date de création**, jamais par dernière modification : la liste bougerait sous
  le curseur à chaque enregistrement automatique.
- CRUD nu dans le dépôt, sans use case : aucun effet de bord. Le dépôt vérifie que la note
  appartient à la vidéo de l'URL (404 sinon).

Écran : onglet **Notes** de la fiche (`ProductionNotesPanel`), liste de fichiers à gauche,
note ouverte à droite (la première d'office au large). Sur mobile, la liste seule, puis la
note avec un retour — bascule en CSS. **L'éditeur reçoit le contenu figé à l'ouverture**
(`useState(note.content)`) et une instance par note (`key`) : `useUpdateProductionNote`
écrit la réponse dans le cache (`setQueryData`, pas d'invalidation), et repasser ce contenu
à l'éditeur lui ferait rejouer un texte plus ancien que la frappe en cours.
...
**Une étape ou une tâche du référentiel ne s'applique qu'aux vidéos créées après elle**
(`applies_from`, migration 33, sur `production_steps` **et** `step_todos` ;
`domain/production/services/appliesTo.ts`). Ajouter « sous-titres » ne doit pas rouvrir
les vidéos déjà faites, ni faire crier l'alerte `production_incomplete` sur toutes les
sorties récentes : une vidéo garde le processus du jour où on l'a créée. Trois règles :

- `applies_from` `NULL` = **toutes les vidéos**. C'est ce qu'a reçu tout l'existant, et
  ce n'est pas `created_at` qui sert de borne : les tâches ont été semées par la
  migration 12, **après** une partie des vidéos, et s'y fier les aurait retirées des
  fiches anciennes. Une création pose `applies_from = created_at` ; les seeds le laissent
  à `NULL`.
- une case **déjà cochée** reste toujours affichée, même hors période ;
- Paramètres → Étapes peut **étendre** une étape ou une tâche à toutes les vidéos
  (`PATCH { appliesFrom: null }`, seule valeur admise par zod), avec confirmation : les
  sorties récentes la signaleront alors comme non cochée.

La règle vit à **deux** endroits côté API, et nulle part côté front :
`SqliteTodoRepository.listForProductions` filtre les tâches (tout l'aval — avancement,
`ManageTodos`, alerte — lit cette liste), et `ProductionView.stepIds` porte les étapes
**actives** qui s'appliquent à la vidéo, dans l'ordre d'affichage. Le front ne lit jamais
le référentiel entier pour une vidéo : `stepsOf(production, steps)` le filtre (pastilles,
`StartTimerDialog`, `AddToPlanDialog`, sélecteur d'étape du temps passé), et
`progressCounts` / `stepProgress` ne prennent plus de `totalSteps` — ils comptent
`stepIds`. L'avancement moyen de `GetProductionOverview.buildStats` suit la même règle.
...
### `postDraft` — les publications à venir
...
formulaire), une date prévue **facultative**, et **six cases** (`POST_DRAFT_STEPS`, liste
fermée dupliquée des deux côtés) : `montage`, `sous_titres`, `miniature`, puis `youtube`,
`instagram`, `tiktok`. Pas de script, pas de créneaux : c'est un outil d'organisation, pas
une production. **Aucun lien avec `ig_media`** : ce qui est paru se lit dans Audience →
Instagram.
...
- **Pastille `-X`** sur l'entrée de menu (`publicationBadge`, `navBadges.ts`) : X = jours
  **locaux** écoulés depuis `lastPublishedAt` (ramené au jour du navigateur par
  `localDateOf`). Rien si une publication a été validée aujourd'hui ; neutre à -1, orange
  dès -2 ; un point si rien n'a jamais été validé. `NavBadge.text` a été ajouté pour
...
`description` n'est jamais `null`. Tri de la liste : `planned_date IS NULL, planned_date,
created_at` ; des archives : `archived_at DESC`. La description se **copie** d'un clic.
CRUD nu sans use case, comme `ideas` : toutes les mutations invalident `['postDrafts']`
(liste, archives et résumé), rien d'autre.
...
#### Poser un créneau à la main depuis la pile

`ManagePlanning.placeItem(itemId, { date, startTime, minutes? })` — le glisser-déposer
d'une ligne de « En cours » vers la grille. Le créneau naît **`manual`** (on vient de
choisir le moment, le prochain « Repositionner » n'a pas à le défaire) et garde son
`itemId`, si bien que le moteur le compte comme couverture. **Rien n'est replanifié.**

**On peut en poser autant qu'on veut sur la même ligne**, y compris quand elle est déjà
entièrement couverte : un montage de trois heures se fait souvent en trois soirées, et un
refus au motif que le compte est bon obligerait à gonfler l'estimation pour contourner
l'outil.

`defaultSlotMinutes(item, sizing)` décide de la durée : le **reste à couvrir** d'abord
(`plannedMinutes − approvedMinutes − scheduledMinutes`), plafonné par `maxBlockMinutes` ;
quand il ne reste rien, `minBlockMinutes`. Elle est **dupliquée à l'identique côté front**,
comme `slotMinutes` — le bloc fantôme qui suit le curseur doit faire exactement la taille
du créneau qui sera posé, sinon il saute de hauteur au relâchement. Le front envoie donc
`minutes` explicitement : entre le début du geste et le lâcher, un autre créneau a pu
changer le reste à couvrir.
...
lance jamais de navigateur. Verrou en mémoire par clé (`CollectIntegrations.running`, 409) : le cron et un clic ne connectent pas deux navigateurs au même compte — c'est
...
| `POST`   | `/api/productions/:id/publish`                      | `{ videoId }` → rattache la sortie, passe en `done`. **Ne coche aucune étape**                                                                                                                                                                           |
| `PUT`    | `/api/productions/:id/steps/:stepId`                | Cocher une étape (idempotent)                                                                                                                                                                                                                            |
| `DELETE` | `/api/productions/:id/steps/:stepId`                | Décocher                                                                                                                                                                                                                                                 |
| `GET`    | `/api/productions/:id/notes`                        | Notes de la vidéo, ordre de création. 404 si la vidéo n'existe pas                                                                                                                                                                                       |
| `POST`   | `/api/productions/:id/notes`                        | `{ title?, content? }` (tout facultatif : « Nouvelle » crée une note vide)                                                                                                                                                                               |
| `PATCH`  | `/api/productions/:id/notes/:noteId`                | `{ title?, content? }`. 404 si la note n'appartient pas à cette vidéo                                                                                                                                                                                    |
| `DELETE` | `/api/productions/:id/notes/:noteId`                | Supprimer une note                                                                                                                                                                                                                                       |
| `GET`    | `/api/production-steps`                             | Référentiel des étapes. Param `includeArchived`                                                                                                                                                                                                          |
| `POST`   | `/api/production-steps`                             | Créer                                                                                                                                                                                                                                                    |
| `PATCH`  | `/api/production-steps/:id`                         | Modifier / archiver / réordonner. `appliesFrom: null` étend l'étape aux vidéos créées avant elle                                                                                                                                                         |
...
| `GET`    | `/api/post-drafts`                                  | La liste, date prévue la plus proche en tête. `?archived=true` : les archives, plus récentes d'abord                                                                                                                                                     |
| `GET`    | `/api/post-drafts/summary`                          | `{ lastPublishedAt, pending }` — la pastille « -X » du menu. **Déclaré avant `/:id`**                                                                                                                                                                    |
| `POST`   | `/api/post-drafts`                                  | `{ title, description?, plannedDate? }`. `description` ≤ 2 200 caractères                                                                                                                                                                                |
| `PATCH`  | `/api/post-drafts/:id`                              | Modifier. `steps` remplace toutes les cases, `archived: true/false` archive ou restaure, `plannedDate: null` efface la date                                                                                                                              |
...
| `POST`   | `/api/planning/slots/:id/continue`                  | « Continuer le travail » (clic droit) : `{ date, startTime, minutes }` **locaux** → nouveau créneau `manual` non approuvé, même vidéo/étape/libellé, ligne de pile gardée si encore ouverte. Ne replanifie rien                                          |
...
défile réellement, et tant que la grille débordait dans le flux de la page, la ligne des
dates partait vers le haut dès qu'on descendait chercher une fin d'après-midi. Le bloc
collant réunit **en-têtes de jours, swimlane et journées entières** — les empiler
séparément demanderait un `top` par bande, recalculé chaque fois qu'une swimlane apparaît.
La gouttière des heures est `sticky left-0` pour la même raison, sur sept colonnes qui
défilent horizontalement.

**Le board garde les données précédentes pendant qu'il charge les suivantes**
(`placeholderData: (previous) => previous`, comme `useAnalytics`). Sans ça, chaque clic de
flèche changeait la clé de cache, la requête repartait de zéro et l'écran se **démontait** :
grille remplacée par « Chargement… », puis tout remonté — un clignotement à chaque pas.

**Une tâche de la pile se glisse sur la grille** pour lui donner une heure à la main. Le
partage des rôles est net et vaut pour tout geste inter-composants : **la pile dit ce
qu'on tient, la grille dit où on le lâche** — elle seule connaît sa géométrie. L'état
`pending` vit donc dans `PlanningPage`, qui monte les deux. Les écouteurs sont posés sur
`window` et non sur la grille : le geste a commencé ailleurs, il n'y a aucun pointeur à
capturer, et n'écouter qu'au-dessus de la grille ferait perdre le fil dès qu'on la survole
de biais. Lâcher hors du cadre ne pose rien — plus clair que de rabattre le bloc sur le
bord le plus proche.

Le fantôme lu au relâchement vient d'une **ref** et non de l'état : la closure de
l'écouteur figerait la valeur au moment de l'attache. Et l'état du fantôme n'est jamais
remis à `null` depuis l'effet — la règle `react-hooks/set-state-in-effect` le refuse ; il
est lu à travers `pendingItem` (`activeGhost`), ce qui revient au même.

**Un créneau se redimensionne par son bord bas**, et seule la **fin** bouge : on rallonge
une séance, on ne la déplace pas — c'est le glissement du bloc qui fait ça. Le
`stopPropagation` sur la poignée est indispensable, sans lui le même geste démarrerait
aussi le déplacement. La poignée est **absente sur un créneau approuvé** : sa durée est
celle de la session de travail enregistrée, et la tirer les ferait diverger sans que le
compteur de la vidéo bouge — le temps réellement passé se corrige sur la fiche.
...
**La grille montre la journée entière, de minuit à minuit** (`DAY_BOUNDS`, qui a remplacé
`dayBounds`). Elle s'ouvrait sur la première plage travaillable : tout ce qui tombait hors
des horaires — une tâche Todo à 7 h, un rendez-vous à 21 h — n'avait nulle part où être
posé. C'est le **défilement à l'ouverture** qui remplace le cadrage : la grille se cale sur
l'heure qu'il est, au premier tiers de la zone visible. Les heures hors plage restent
teintées.

**Le zoom est une hauteur d'heure**, par crans (`ZOOM_LEVELS`, 24 → 140 px, défaut
`DEFAULT_ZOOM` = 56 px), réglé par les boutons − / % / + de la barre (montés au large et
dans le menu mobile) et mémorisé en préférence (`planningZoom`). En changeant de cran, **le
moment au centre de l'écran reste au centre** — sinon tout s'étirerait depuis minuit et la
vue repartirait vers le matin. Les deux défilements vivent dans un seul `useLayoutEffect`
qui garde le cran précédent en ref.

**Au large, la grille descend jusqu'au bas de l'écran.** Sa hauteur est **mesurée**
(`window.innerHeight` moins son `top` moins `BOTTOM_GAP`) et posée sur le style de
l'élément, sans état React — un `ResizeObserver` sur `document.body` suit l'apparition d'un
bandeau. Une constante (`calc(100vh-13rem)`) laissait un vide sous la grille. Sur mobile, la
hauteur reste le `calc(100dvh - …)` déjà exact.

**Les blocs qui se chevauchent se rangent côte à côte** (`layoutLanes`, domaine front) :
grappes de blocs qui se recouvrent de proche en proche, première colonne libre, largeur
partagée par la grappe. Créneaux et tâches Todo placées y passent **ensemble**, avec leur
fin **visuelle** (un bloc de 5 min occupe 20 px à l'écran). Les événements d'agenda restent
en fond, pleine largeur.

**Le clic droit sur un créneau ouvre « Continuer le travail »** (menu maison en portail,
fermé au clic ailleurs, à `Échap`, au défilement). Il crée un créneau d'**une heure,
maintenant** (quart d'heure entamé, borné à minuit), sur la même vidéo, étape et libellé —
approuvé ou non, puisque c'est en fin de séance qu'on l'enchaîne. Il naît non approuvé mais
**`manual`** : `planner`, le prochain « Repositionner » l'éloignerait de l'instant demandé.
Il garde la ligne de pile d'origine **si elle est encore ouverte**. Le glissement ignore
désormais tout bouton autre que le principal (`event.button !== 0`), sans quoi le clic droit
démarrait aussi un déplacement.

**La rangée des tâches Todo se replie** (icône ▾ de la gouttière, préférence
`planningTodosCollapsed`) sur un compteur par jour (« 3 à faire · 1 en retard »).
...
ce que la barre d'application affiche déjà. Le sous-titre, lui, _explique l'écran_ (« Une
...
- **Migration 33** ajoute `applies_from` à `production_steps` et `step_todos` (`NULL` pour
  l'existant = toutes les vidéos) : une étape ou une tâche créée ensuite ne s'applique
  qu'aux vidéos créées après elle.
...
- **L'alerte `production_incomplete` ne compte que les étapes de la vidéo** (`stepIds`), sans
  les étapes archivées : elle parcourait avant `findAll(true)`, si bien qu'une étape
  archivée ou ajoutée après coup apparaissait comme « non cochée » sur toutes les sorties
  récentes.
...
- **Le badge de statut d'une carte de file est un menu, pas une étiquette**
  ...
  aucune sortie** : c'est `PublishDialog`, depuis la fiche, qui relie la vidéo collectée
  (sans cocher aucune étape). L'entrée le dit ; l'interdire obligerait à passer par la
...
- **Poser un créneau à la main ne replanifie rien, et ne se refuse jamais.** On peut en
  poser plusieurs sur la même ligne de pile même si le compte de minutes est déjà bon :
  c'est le geste normal quand un montage s'étale sur trois soirées. La durée retombe alors
  sur `minBlockMinutes`.
- **La durée d'un bloc posé à la main est calculée des DEUX côtés** (`defaultSlotMinutes`,
  dupliquée) : le fantôme qui suit le curseur doit faire exactement la taille du créneau
  qui naîtra, sinon il saute de hauteur au relâchement. Le front envoie `minutes` plutôt
  que de laisser l'API recalculer — entre le début du geste et le lâcher, le reste à
  couvrir a pu changer.
- **Le redimensionnement ne touche que la fin du créneau**, et pas les approuvés. Ne pas y
  ajouter de poignée haute : déplacer le début est déjà ce que fait le glissement du bloc,
  et deux gestes pour le même résultat finiraient par se contredire.
