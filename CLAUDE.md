...
- **`AudienceChart` calcule le domaine de l'axe des abonnés cumulés en JavaScript.** `subscribersTotal` vaut `null` sur les buckets antérieurs au premier relevé ; avec `domain={['dataMin - 100', 'dataMax + 100']}`, Recharts calculait ses bornes sur une série contenant des `null`, partait en `NaN` et **ne dessinait plus l'aire du tout**. Les trois autres métriques n'ont jamais de trou, ce qui explique que seule celle-ci cassait. Ne pas remettre de domaine en chaîne de caractères sur une série qui peut contenir des `null`.
...
- **Le tableau de vidéos est trié par en-tête cliquable** (`VideoPerformanceTable`), premier clic décroissant : sur des vues ou des euros, c'est presque toujours ce qu'on cherche. Le calcul du montant par vidéo vit dans `charts/videoPerformance.ts` (`withMoney`, `sumVideoRows`), partagé par les barres et le tableau — les deux doivent afficher le même total pour la même vidéo.
