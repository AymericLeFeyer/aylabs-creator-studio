 ...
 * (`graph.instagram.com`), connexion directe sans Page Facebook — jetons préfixés
 * `IGAA…`. L'ancien flux « … with Facebook Login » (`graph.facebook.com`, Page requise,
 * `META_APP_ID`/`META_APP_SECRET`) a été **retiré** : ce studio ne s'en sert pas, et le
 * garder pour un cas jamais utilisé aurait doublé chaque branchement pour rien.
 *
...
export class InstagramClient {
  ...
  private async call<T>(path: string, params: Record<string, string> = {}): Promise<T> {
    const url = new URL(`${GRAPH_INSTAGRAM}${path}`);
   ...
   * s'arrête au bout de deux mois. `graph.instagram.com/refresh_access_token`, **sans
   * identifiant d'app** — le jeton de connexion directe se suffit à lui-même. Seule
   * condition posée par Meta : il doit avoir au moins 24 h d'existence.
   */
