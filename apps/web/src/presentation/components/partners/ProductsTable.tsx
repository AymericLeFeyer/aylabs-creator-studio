...
/**
 * La table des produits, **avec tout ce qui la pilote** : la case « Reste à faire
 * uniquement », le total qui change de nature avec elle, l'ajout et la modale d'édition.
 * Un bloc autonome, pour pouvoir la poser sur le dashboard telle quelle.
 *
 * Filtré ici et non dans l'API : `/api/products` alimente aussi les sélecteurs de
 * rattachement, qui doivent tout voir. « Reste à faire » **ignore la période** : un produit
 * reçu en mars et pas encore tourné est toujours à tourner en août.
 */
export const ProductsTable = () => {
  ...
  const stats = useMemo(
    () => ({ pipeline: partnerPipeline(products, [], toIsoDate(new Date()), range) }),
    [products, range],
  );
